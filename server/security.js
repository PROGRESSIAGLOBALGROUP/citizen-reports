...
const CONFIG = {
  ...
  // Datos sensibles a cifrar (PII - Información Personal Identificable)
  // Incluye: datos de contacto, ubicación precisa, identificadores de dispositivo
  SENSITIVE_FIELDS: [
    // Datos de contacto
    'descripcion', 'notas', 'direccion', 'colonia', 'nombre_ciudadano', 'telefono', 'email_ciudadano',
    // Datos de ubicación/red (PII bajo GDPR/LFPDPPP)
    'ip_cliente', 'ip', 'fingerprint', 'user_agent'
  ]
};
...
export function registrarIntentoFallido(req) {
  ...
  if (!record || (now - record.firstAttempt) > CONFIG.LOGIN_WINDOW_MS) {
    ...
    if (record.attempts >= CONFIG.LOGIN_MAX_ATTEMPTS) {
      ...
      console.log(`⚠️ IP ${ip} bloqueada por ${CONFIG.LOGIN_BLOCK_DURATION_MS / 60000} minutos (${record.attempts} intentos fallidos)`);
...
function getEncryptionKey() {
  ...
  return crypto.createHash('sha256').update('citizen-reports-dev-key-change-in-production').digest();
}
...
export function hashForSearch(value) {
  ...
  return crypto.createHash('sha256').update(value.toLowerCase().trim()).digest('hex').substring(0, 16);
}
...
export function csrfProtection(req, res, next) {
  ...
  // Por ahora, solo advertir si no hay CSRF token (para no romper frontend existente)
  if (!csrfToken) {
    console.warn(`⚠️ Request sin CSRF token: ${req.method} ${req.path}`);
    // TODO: Descomentar cuando frontend envíe tokens CSRF
    // return res.status(403).json({ error: 'Token CSRF requerido', codigo: 'CSRF_REQUIRED' });
  }
...
// Ejecutar limpieza cada 5 minutos (solo en producción/desarrollo, no en tests)
let sessionCleanupInterval = null;
if (process.env.NODE_ENV !== 'test') {
  sessionCleanupInterval = setInterval(limpiarSesionesInactivas, 5 * 60 * 1000);
}

/**
 * Detener el intervalo de limpieza de sesiones (útil para tests)
 */
...
export function decryptFile(encryptedBuffer) {
  ...
  const authTag = encryptedBuffer.slice(CONFIG.IV_LENGTH, CONFIG.IV_LENGTH + CONFIG.AUTH_TAG_LENGTH);
