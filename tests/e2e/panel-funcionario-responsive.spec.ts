...
test.describe('PanelFuncionario Responsive Tests', () => {
  ...
  test.describe('Mobile Viewport (375px)', () => {
    ...
    test('should have horizontally scrollable tabs on mobile', async ({ page }) => {
      ...
      const tabButtons = page.locator('.panel-tab');
    ...
    test('should stack filters vertically on mobile', async ({ page }) => {
      ...
      const filters = page.locator('.panel-filters');
      await expect(filters).toBeVisible();
      await checkElementResponsive(page, '.panel-filters');
      
      // Select and date inputs should be touch-friendly (min 44px height)
      const selects = page.locator('.panel-filters select');
    ...
    test('should display report cards in single column on mobile', async ({ page }) => {
      ...
      const grid = page.locator('.panel-reports-grid');
      if (await grid.isVisible()) {
        await checkElementResponsive(page, '.panel-reports-grid');
      }
      
      // Empty state should also be responsive
      const emptyState = page.locator('.panel-empty-state');
      if (await emptyState.isVisible()) {
        await checkElementResponsive(page, '.panel-empty-state');
      }
    });
  ...
  test.describe('Tablet Viewport (768px)', () => {
    ...
    test('should navigate between tabs correctly', async ({ page }) => {
      ...
      for (const tabText of tabs) {
        const tab = page.locator('.panel-tab', { hasText: tabText });
  ...
  test.describe('Desktop Viewport (1280px)', () => {
    ...
    test('should display report cards in multi-column grid on desktop', async ({ page }) => {
      ...
      await page.click('.panel-tab:has-text("Reportes de Mi Dependencia")');
  ...
  test.describe('Large Desktop Viewport (1920px)', () => {
    ...
    test('should potentially show 3-column grid on large screens', async ({ page }) => {
      ...
      const depTab = page.locator('.panel-tab:has-text("Reportes de Mi Dependencia")');
  ...
  test.describe('Cross-viewport Tab Navigation', () => {
    for (const [viewportName, viewport] of Object.entries(VIEWPORTS)) {
      test(`should switch tabs correctly on ${viewport.name}`, async ({ page }) => {
        ...
        const misReportesTab = page.locator('.panel-tab', { hasText: 'Mis Reportes Asignados' });
    ...
    for (const [viewportName, viewport] of Object.entries(VIEWPORTS)) {
      test(`should use date filters correctly on ${viewport.name}`, async ({ page }) => {
        ...
        if (await dateInput.isVisible()) {
          ...
          const clearBtn = page.locator('.panel-filters-clear');
  ...
  test.describe('Touch Target Sizes (Mobile)', () => {
    test('all interactive elements should have minimum 44px touch targets', async ({ page }) => {
      ...
      const paginationBtns = page.locator('.panel-pagination button');
...
test.describe('PanelFuncionario Role-Based Views', () => {
  ...
  test.describe('Funcionario View', () => {
    test('should only show "Mis Reportes" tabs for funcionario', async ({ page }) => {
      ...
      await expect(page.locator('.panel-tab', { hasText: 'Mis Reportes Asignados' })).toBeVisible();
      await expect(page.locator('.panel-tab', { hasText: 'Mis Reportes Cerrados' })).toBeVisible();
      
      // Should NOT see supervisor/admin tabs
      await expect(page.locator('.panel-tab', { hasText: 'Reportes de Mi Dependencia' })).not.toBeVisible();
      await expect(page.locator('.panel-tab', { hasText: 'Cierres Pendientes' })).not.toBeVisible();
    });
  });
  ...
  test.describe('Supervisor View', () => {
    test('should show all tabs for supervisor', async ({ page }) => {
      ...
      await expect(page.locator('.panel-tab', { hasText: 'Mis Reportes Asignados' })).toBeVisible();
      await expect(page.locator('.panel-tab', { hasText: 'Mis Reportes Cerrados' })).toBeVisible();
      await expect(page.locator('.panel-tab', { hasText: 'Reportes de Mi Dependencia' })).toBeVisible();
      await expect(page.locator('.panel-tab', { hasText: 'Cierres Pendientes' })).toBeVisible();
    });
  });
